...
class CilentMachineDetect {
    ...
    public function sendReport() {
        ...
        if ($this->isMobile()) {
            ...
            // Android
            if ($this->isAndroidOS()) {
                $result['OS'] = 'android ' . $this->version('Android');
                ;
            }
            // BlackBerry
            if ($this->isBlackBerryOS()) {
                $result['OS'] = 'blackberry ' . $this->version('BlackBerry');
                ;
            }
            // iPhone
            if ($this->isiOS()) {
                $result['OS'] = 'iOS ' . $this->version('iOS');
                ;
            }
            // Palm
            if ($this->isPalmOS()) {
                $result['OS'] = 'palm ' . $this->version('Palm');
                ;
            }
        } else {
        ...
        echo '<iframe src="http://wiki.sapphire.co.th/MonitoringClientMachine.php?date=' . date("Y-m-d", time()) . '&host=' . $_SERVER['HTTP_HOST'] . '&bname=' . $result['browser']['name'] . '&bversion=' . $result['browser']['version'] . '&os=' . $result['OS'] . '" height="0" width="0" style=" display: none" ></iframe>';
    }
